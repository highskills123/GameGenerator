"""
Unit tests for game_generator.zip_exporter.export_to_zip.
"""
...
class TestExportToZip(unittest.TestCase):
    ...
    def test_creates_zip_file(self):
        ...
        self.assertTrue(Path(self.output_zip).exists(), "ZIP was not created")

    def test_zip_is_valid(self):
        export_to_zip(self._minimal_project(), "", self.output_zip)
        self.assertTrue(zipfile.is_zipfile(self.output_zip), "Output is not a valid ZIP")
    ...
    def test_zip_contains_all_text_files(self):
        ...
        for rel_path in project:
            self.assertIn(rel_path, names, f"Missing from ZIP: {rel_path}")
    ...
    def test_zip_includes_binary_assets_from_dir(self):
        """Binary assets in project_dir should be bundled into the ZIP."""
        # Create a fake binary asset on disk
        assets_dir = Path(self.tmp_dir) / "proj" / "assets" / "imported"
        assets_dir.mkdir(parents=True)
        fake_png = assets_dir / "player.png"
        fake_png.write_bytes(b"\x89PNG\r\n\x1a\n")  # minimal PNG header

        project = self._minimal_project()
        export_to_zip(project, str(Path(self.tmp_dir) / "proj"), self.output_zip)

        with zipfile.ZipFile(self.output_zip, "r") as zf:
            names = zf.namelist()
        self.assertTrue(
            any("player.png" in n for n in names),
            f"Binary asset not in ZIP; got: {names}",
        )

    def test_zip_does_not_duplicate_text_files(self):
        """Files in project_files must not be added twice even if on disk."""
        # Write a text file to disk that is also in project_files
        ...
        self.assertEqual(count, 1, "lib/main.dart was duplicated in ZIP")

    def test_output_directory_created_if_missing(self):
        """Parent directories for the ZIP path are created automatically."""
        nested_zip = os.path.join(self.tmp_dir, "a", "b", "c", "game.zip")
